// ─── Stevia Care — AI Service (Direct Groq) ──────────────────────────────
// Calls Groq API directly from app — fast, always works, no server needed
...
// ── Core caller ───────────────────────────────────────────────────────────
async function callGroq(system, userContent, maxTokens = 2000) {
...
async function callGroqChat(system, messages, maxTokens = 1500) {
  const key = GROQ_API_KEY;
  if (!key || key === 'YOUR_GROQ_API_KEY_HERE' || key === '') {
